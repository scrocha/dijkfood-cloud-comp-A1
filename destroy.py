...
REPO_NAMES = [
    "dijkfood-api-cadastro",
    "dijkfood-api-rotas",
    "dijkfood-api-pedidos",
]
SERVICES = [
...
SIM_REPO_NAMES = [
    "dijkfood-sim-gateway",
    "dijkfood-sim-clientes",
    "dijkfood-sim-restaurante",
    "dijkfood-sim-entregadores",
    "dijkfood-sim-completo",
    "dijkfood-sim-carga",
]
SIM_SERVICES = [
    "dijkfood-sim-gateway-svc",
    "dijkfood-sim-clientes-svc",
    "dijkfood-sim-restaurante-svc",
    "dijkfood-sim-entregadores-svc",
]
SIM_ALB_NAME = "dijkfood-sim-alb"
SIM_TG_NAMES = [
    "df-sim-tg-gateway",
    "df-sim-tg-clientes",
    "df-sim-tg-restaurante",
    "df-sim-tg-entregadores",
]
...
def delete_ecs_resources():
    ...
    for service in SERVICES:
        ...
        try:
            ...
            ecs.delete_service(
                cluster=CLUSTER_NAME, service=service, force=True
            )
    ...
    if not endpoint:
        try:
            ...
            if instance["DBInstanceStatus"] != "available":
                print(
                    f"RDS não está disponível (Status: {instance['DBInstanceStatus']}). Pulando limpeza."
                )
                return
            endpoint = instance["Endpoint"]["Address"]
        except Exception:
            print(
                "Não foi possível obter endpoint do RDS via API. Pulando limpeza de dados."
            )
            return
...
def delete_simulator_resources():
    ...
    for service in SIM_SERVICES:
        try:
            ecs.update_service(
                cluster=SIM_CLUSTER_NAME, service=service, desiredCount=0
            )
            ecs.delete_service(
                cluster=SIM_CLUSTER_NAME, service=service, force=True
            )
    ...
    try:
        ...
        for task_arn in task_arns:
            ecs.stop_task(
                cluster=SIM_CLUSTER_NAME, task=task_arn, reason="Destroy"
            )
        if task_arns:
...
def main():
    ...
    parser.add_argument(
        "--hard",
        action="store_true",
        help="Deleta RDS, ECR e Security Group permanentemente",
    )
