...
NUM_USUARIOS = int(os.environ.get("SEED_USERS", "1000"))
NUM_ENTREGADORES = int(os.environ.get("SEED_DRIVERS", "3000"))
...
CARDAPIO = {
    ...
    "Japonesa": [
        ...
        "Temaki de Atum",
        "Ceviche Oriental",
        "Teppanyaki de Mignon",
        "Yakitori de Frango",
        "Udon de Frutos do Mar",
        "Mochi de Morango",
    ],
    "Brasileira": [
        "Feijoada Completa",
...
def gerar_coordenadas_validas_sp(quantidade):
    ...
    print(
        f"Gerando {quantidade} pontos estritamente dentro da área suportada pelas Rotas..."
    )
    ...
    for ponto in multiponto.geoms:
        ...
        if (-23.9857223 <= lat <= -23.3590754) and (
            -46.8253578 <= lon <= -46.3653906
        ):
    ...
    while len(coordenadas) < quantidade:
        coordenadas.append(
            coordenadas[0] if coordenadas else (-23.5505, -46.6333)
        )
...
async def esperar_api_pronta(client, url, max_retries=30, delay=10):
    print(
        f"Aguardando a API ({url}) ficar completamente pronta no Target Group..."
    )
    ...
    for i in range(max_retries):
        try:
            ...
            pass
        print(
            f"API ainda não responde (Tentativa {i + 1}/{max_retries}). Aguardando {delay}s..."
        )
        await asyncio.sleep(delay)
    else:
        print(
            "Aviso: Limite de tempo esgotado aguardando API. Tentando prosseguir mesmo assim."
        )
        return False
    ...
    for i in range(15):
        try:
            resp = await client.get(
                f"{url}/cadastro/restaurantes", timeout=15.0
            )
            if resp.status_code < 500:
                print(
                    "A API está online e pronta para receber as requisições!"
                )
                return True
        except Exception:
            pass
        print(
            f"Endpoint de dados ainda com 502 (Tentativa {i + 1}/15). Aguardando 5s..."
        )
        await asyncio.sleep(5)

    print(
        "Aviso: Endpoints de dados não responderam. Tentando seed mesmo assim."
    )
    return False
...
async def send_batch(client, endpoint, payload, retries=5):
    ...
    for i in range(retries):
        try:
            ...
            if i < retries - 1:
                print(
                    f"Aviso: Erro ao enviar para {endpoint}: {e}. Retentando em 5s (Tentativa {i + 1})..."
                )
                await asyncio.sleep(5 * (2**i))
            else:
                print(
                    f"Erro definitivo ao enviar para {endpoint} após {retries} tentativas: {e}"
                )
...
async def seed_entregadores(client, total):
    ...
    for batch_start in range(0, total, DRIVER_BATCH_SIZE):
        batch_data = []

        for _ in range(min(DRIVER_BATCH_SIZE, total - batch_start)):
            lat, lon = next(COORDENADAS)
        ...
        await send_batch(
            client, "/pedidos/drivers/batch-location", dynamo_payload
        )
...
async def seed_produtos(client, rest_ids, cozinhas):
    print("Iniciando carga de produtos para os restaurantes...")
    batch_data = []
    total_inseridos = 0
...
async def run_seed():
    global COORDENADAS
    COORDENADAS = iter(
        gerar_coordenadas_validas_sp(
            NUM_USUARIOS + NUM_ENTREGADORES + NUM_RESTAURANTES
        )
    )
