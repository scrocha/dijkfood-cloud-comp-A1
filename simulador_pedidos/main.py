...
from simulador_pedidos.models import StartSimulationRequest, CheckoutRequest, ItemPedido
...
app = FastAPI(title="DijkFood - Simulador de Pedidos de Clientes", lifespan=lifespan)

async def _obter_dados_reais():
    """Busca dados na API Geral para popular a simulação"""
    try:
        # Busca Clientes
        resp_usuarios = await sim_state.http_client.get(f"{GENERAL_API_URL}/usuarios")
        usuarios = resp_usuarios.json()
        
        # Busca Restaurantes
        resp_restaurantes = await sim_state.http_client.get(f"{GENERAL_API_URL}/restaurantes?page_size=100")
    ...
    while sim_state.is_running:
        try:
            ...
            if not usuarios or not restaurantes:
                print("[Simulador] Sem dados suficientes (usuarios/restaurantes). Aguardando...")
            ...
            rest_id = restaurante.get("id") or restaurante.get("restaurante_id")
            resp_itens = await sim_state.http_client.get(f"{GENERAL_API_URL}/restaurantes/{rest_id}/itens")
            ...
            itens_escolhidos = random.sample(itens_disponiveis, min(num_itens, len(itens_disponiveis)))
            ...
            for item in itens_escolhidos:
                ...
                pid = item.get("id") or item.get("produto_id")
            ...
            # Monta o Checkout
            checkout_data = {
                "customer_id": str(usuario.get("id") or usuario.get("usuario_id")),
            ...
            print(f"[Simulador] Enviando pedido: Cliente {checkout_data['customer_id']} -> Rest {rest_id}")
            resp_checkout = await sim_state.http_client.post(
                f"{GENERAL_API_URL}/checkout", 
                json=checkout_data
            )
            
            if resp_checkout.status_code >= 400:
                print(f"[Simulador] Erro no Checkout ({resp_checkout.status_code}): {resp_checkout.text}")
