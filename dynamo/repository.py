...
class OrderRepository:
    def __init__(self):
        ...
        if os.getenv("DYNAMODB_AUTO_CREATE", "0").lower() in {
            "1",
            "true",
            "yes",
            "y",
    ...
    def create_order(self, order_data: OrderCreate):
        ...
        self.client.transact_write_items(
            TransactItems=[
                {
                    "Put": {
                        "Item": self._to_dynamo_dict(metadata_item),
                        "TableName": self.table_name,
                    }
                },
                {
                    "Put": {
                        "Item": self._to_dynamo_dict(history_item),
                        "TableName": self.table_name,
                    }
                },
            ]
    ...
    def update_status(
        ...
        if next_status not in ORDER_FLOW.get(current_status, []):
            raise ValueError(
                f"Transição de {current_status} para {next_status} não é permitida."
            )

        # Monta a expressão de atualização para os metadados
        update_expr = (
            "SET #s = :new_status, updated_at = :now, GSI2PK = :new_gsi2pk"
        )
        expr_attr_values = {
        ...
        if next_status == OrderStatus.PREPARING:
            if not final_entregador_id:
                raise ValueError(
                    "entregador_id é obrigatório ao transicionar para PREPARING, pois ele acabou de ser associado."
                )
            ...
            transact_items.append(
                {
                    "Update": {
                        "TableName": self.table_name,
                        "Key": {
                            "PK": {"S": f"DRIVER#{final_entregador_id}"},
                            "SK": {"S": "LATEST"},
                        },
        ...
        try:
            ...
            if any(r.get("Code") == "ConditionalCheckFailed" for r in reasons):
                ...
                if reasons[0].get("Code") == "ConditionalCheckFailed":
                    raise ValueError(
                        f"Pedido {order_id} já mudou de status ou não está mais em {current_status}."
                    )
                if (
                    len(reasons) > 2
                    ...
                    raise ValueError(
                        f"Entregador {final_entregador_id} não está disponível (LIVRE)."
                    )
            raise e
    ...
    def _sync_to_postgres(self, order_id: str):
        ...
        try:
            ...
            payload = {
                ...
                "entregador_id": metadata.get(
                    "entregador_id", "SEM_ENTREGADOR"
                ),
                "confirmed_time": status_times.get("CONFIRMED", fallback),
                "preparing_time": status_times.get("PREPARING", fallback),
                "ready_for_pickup_time": status_times.get(
                    "READY_FOR_PICKUP", fallback
                ),
            ...
            cadastro_base = os.getenv(
                "CADASTRO_SERVICE_URL", "http://database-service:8000"
            ).rstrip("/")
            ...
            with httpx.Client() as client:
                ...
                print(
                    f"Pedido {order_id} sincronizado com PostgreSQL com sucesso."
                )
...
class LocationRepository:
    CACHE_PREFIX = "CACHE#FREE_DRIVERS"
    ...
    def update_driver_location(
        self, driver_id: str, lat: float, lng: float, order_id: str = None
    ):
        now = datetime.now(timezone.utc).isoformat()
    ...
    def _query_free_drivers(self, limit: int):
        query_kwargs = {
            "IndexName": "StatusIndex",
            "KeyConditionExpression": Key("GSI2PK").eq(
                f"DRIVER_STATUS#{DriverStatus.LIVRE.value}"
            ),
            "Limit": limit,
        }
    ...
    def _lock_key(self, limit: int) -> dict:
        return {"PK": f"{self.CACHE_PREFIX}#{limit}", "SK": "LOCK"}

    def get_free_drivers(self, limit: int = 50, cache_ttl_seconds: int = 3):
        now = int(time.time())
        cache_key = self._cache_key(limit)
        lock_key = self._lock_key(limit)

        try:
            cached = self.table.get_item(Key=cache_key).get("Item")
        ...
        if not lock_acquired:
            for _ in range(5):
                ...
                try:
                    cached = self.table.get_item(Key=cache_key).get("Item")
                    if cached and int(cached.get("expires_at", 0)) > int(
                        time.time()
                    ):
                        drivers = cached.get("drivers", [])
                        for item in drivers:
            ...
            return self._query_free_drivers(limit)
        ...
        try:
            ...
            try:
                self.table.delete_item(Key=lock_key)
            except Exception:
                pass
